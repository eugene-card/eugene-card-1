...
(function () {
  ...
  async function sync(reason='manual') {
    ...
    syncInFlight = (async () => {
      try {
        ...
        if(window.auth) window.auth.currentUser=u;
        if(u){
          const p=await ensureProfile(u);
          if(p){u.profile=p;u.username=p.username;u.name=p.name;u.displayName=p.name || u.displayName;u.avatarUrl=p.avatarUrl || u.photoURL;u.bio=p.bio;u.isPlusMember=p.isPlusMember;u.isAdmin=p.isAdmin || isAdminEmail(u.email);window.currentUser=u;if(window.auth)window.auth.currentUser=u;}
        }
  ...
  function renderLoggedInUI(u){
    ...
    buttons.forEach(el=>{
      ...
      const item=(label,fn)=>{const b=document.createElement('button');b.type='button';b.textContent=label;b.style.cssText='display:block;width:100%;text-align:left;padding:10px;border:0;background:transparent;color:#f8fafc;border-radius:9px;cursor:pointer';b.onclick=()=>{menu.style.display='none';fn()};return b;};
  ...
  // Supabase's persisted session is authoritative. INITIAL_SESSION is emitted on every
  // page load, including a normal refresh and browser back/forward restoration.
  sb.auth.onAuthStateChange((event,session)=>{
    const u=authUser(session?.user||null);
    window.currentUser=u;
    if(window.auth)window.auth.currentUser=u;
    setTimeout(()=>sync(event || 'auth-change'),0);
  });

  // Finish an OAuth callback without leaving auth parameters in browser history.
  if (window.location.search.includes('code=')) {
    history.replaceState({}, document.title, window.location.pathname + window.location.hash);
  }

  const boot=()=>{profileModal();sync('boot');};
  ...
  setInterval(()=>{if(!document.hidden)sync('periodic')},30000);
  sb.channel('eugene-card-profile-sync').on('postgres_changes',{event:'*',schema:'public',table:'profiles'},()=>sync('profile-realtime')).subscribe();
})();
