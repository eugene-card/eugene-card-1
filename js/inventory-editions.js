 ...
 * Turns the Inventory > Edit Card Edition field into a persistent dropdown.
 * Admins can add new editions without editing every card manually.
 */
(function () {
  'use strict';
  ...
  function dbRef() {
    try {
      if (window.db && typeof window.db.collection === 'function') return window.db.collection(CONFIG_COLLECTION).doc(CONFIG_DOC);
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `
      <div class="edition-dialog bg-slate-900 border border-violet-500/30 rounded-3xl p-5 space-y-4 relative shadow-2xl">
        <button type="button" class="edition-close absolute top-3 right-3 w-9 h-9 rounded-xl bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center" aria-label="Close">
        ...
        <button type="button" id="inventory-save-edition-btn" class="w-full py-3 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-black text-xs transition-all">
          <i class="fa-solid fa-plus mr-1"></i> Add Edition
        </button>
      </div>`;
  ...
  async function loadEditions() {
    const ref = dbRef();
    const inventoryValues = getInventoryEditions();
    editions = unique(['Beta Edition: #0', ...inventoryValues]);
    ...
    if (ref) {
      try {
        ...
        console.warn('[Eugene Card] Could not load saved editions:', error);
      }
    }
    renderDropdown();
    ready = true;
  ...
  function openAddEdition() {
    if (!window.currentUser || !window.currentUser.isAdmin) {
      return showToastSafe('Admin permission required to add an edition.');
    }
    ensureModal();
  ...
  async function addEdition() {
    if (!window.currentUser || !window.currentUser.isAdmin) return showToastSafe('Admin permission required to add an edition.');
    const input = document.getElementById('inventory-new-edition-input');
    ...
    if (editions.some(existing => existing.toLowerCase() === value.toLowerCase())) {
      renderDropdown(value);
      document.getElementById(SELECT_ID).value = editions.find(existing => existing.toLowerCase() === value.toLowerCase());
      closeAddEdition();
    ...
    try {
      ...
      editions = editions.filter(existing => existing !== value);
  ...
  function init() {
    ...
    // The Inventory modal is already present in index.html, but this also
    // handles future dynamic re-renders without creating a second field.
    const observer = new MutationObserver(() => transformEditionField());
    observer.observe(document.body, { childList: true, subtree: true });

    // Existing openInventoryModal() populates the field after this script has
    // initialized, so keep the option list synchronized with live inventory.
  ...
  window.EugeneInventoryEditions = {
    get: () => [...editions],
    add: addEdition,
    openAddEdition,
    closeAddEdition,
    refresh: refreshForInventoryModal
  };
