...
function json(res, status, body, extraHeaders = {}) {
  res.status(status).set({ 'Cache-Control': 'no-store', Pragma: 'no-cache', ...extraHeaders }).json(body);
}
...
function basicAuth(req) {
  ...
  const i = raw.indexOf(':');
  return i < 0 ? null : { client_id: raw.slice(0, i), client_secret: raw.slice(i + 1) };
}
...
function issueAccessToken() { return randomToken(32); }
function issueRefreshToken() { return randomToken(48); }
function scopes(value) { return String(value || '').trim().split(/\s+/).filter(Boolean); }
function validRedirect(client, redirectUri) { return Array.isArray(client.redirect_uris) && client.redirect_uris.includes(redirectUri); }
